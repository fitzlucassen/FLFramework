...
class App {
    ...
    public function run(){  
        ...
        if($e->getType() == Adapter\ConnexionException::NO_HEADER_TABLE_FOUND){
        ...
        else if($e->getType() == Adapter\ConnexionException::NO_URL_REWRITING_FOUND){
        ...
        if(!$this->_isInErrorPage){
            ...
            foreach($langs as $thisLang){
                ...
                if((!$routes = Core\Cache::read("routeurl")) && self::$_databaseNeeded){
                    $routes = Data\Repository\RouteUrlRepository::getAll($this->_pdo);
                    // On ecrit le résultat en cache
                    Core\Cache::write("routeurl", $routes);
                }
                if(self::$_databaseNeeded){
                    Core\Router::AddRange($routes, $thisLang->getCode(), $this->_pdo);
                }
    ...
    public function Manage404Route(){
        ...
        if((!$this->_isValidUrl && (!self::$_databaseNeeded || !self::$_urlRewritingNeeded)) || (!$this->_isValidUrl && $this->_routeUrl->getId() == 0) || ($this->_url["debug"] == "default" && $this->_page != '/')) {
            Core\Logger::write("Redirection vers la page 404 sur l'url : " . $this->_page);
            ...
            if((!self::$_databaseNeeded || !self::$_urlRewritingNeeded) || $this->_routeUrl->getId() == 0){
                ...
                header('location:' . Core\Router::ReplacePattern($this->_rewrittingUrl->getUrlMatched(), $this->_page));
                die();
            }
        }
    }
    ...
    public function ExecuteAction(){
        ...
        if(!method_exists($this->_controllerName, $this->_actionName)){
            if(self::$_isDebugMode) {
                throw new Adapter\ControllerException(Adapter\ControllerException::ACTION_NOT_FOUND, array("controller" => $this->_url['controller'], "action" => $this->_url['action']));
            }
            else{
                header('location: /home/error404');
                die();
    ...
    public function ManageModuleException(){
        ...
        if((self::$_isDebugMode && self::$_urlRewritingNeeded && self::$_databaseNeeded && !$this->_isInErrorPage)){
            if(!$this->_pdo->TableExist("header")){
                throw new Adapter\ConnexionException(Adapter\ConnexionException::getNO_HEADER_TABLE_FOUND(), null);
            }
            if(!$this->_pdo->TableExist("routeurl") && !$this->_pdo->TableExist("rewrittingurl")){
                throw new Adapter\ConnexionException(Adapter\ConnexionException::getNO_URL_REWRITING_FOUND(), null);
            }
            if(!$this->_pdo->TableExist("lang")){
                throw new Adapter\ConnexionException(Adapter\ConnexionException::getNO_LANG_FOUND(), null);
            }
        }
    }
